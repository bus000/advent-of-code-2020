...
handleInput :: [Int] -> IO ()
handleInput = mapM_ print . take 1 . drop 2019 . playGame

playGame :: [Int] -> [Int]
playGame start = start ++ L.unfoldr go (reverse start)
  where
    go [] = Nothing
    go (x:xs) = case L.elemIndex x xs of
        Just i -> Just (i+1, i+1:x:xs)
        Nothing -> Just (0, 0:x:xs)
