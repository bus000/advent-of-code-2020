...
handleInput :: (PublicKey, PublicKey) -> IO ()
handleInput = print . uncurry computeEncryptionKey

computeEncryptionKey :: PublicKey -> PublicKey -> EncryptionKey
computeEncryptionKey p1 p2
    | p1 < p2 = computePublicKey (determineLoopSize p1) p2
    | otherwise = computePublicKey (determineLoopSize p2) p1

determineLoopSize :: PublicKey -> Int
determineLoopSize key = head . filter p $ [1..]
  where
    p loopSize = computePublicKey loopSize 7 == key

computePublicKey :: Int -> Int -> Int
computePublicKey loopSize subjectNumber = go loopSize 1
  where
    go 0 x = x
    go n x = go (n-1) $ (x * subjectNumber) `mod` 20201227

parseInput :: T.Text -> Either P.ParseError (PublicKey, PublicKey)
parseInput = P.parse (parseKeys <* P.eof) ""

parseKeys :: P.Parsec T.Text () (PublicKey, PublicKey)
parseKeys = (,) <$> parseKey <* P.newline <*> parseKey <* P.newline

parseKey :: P.Parsec T.Text () PublicKey
parseKey = P.int
