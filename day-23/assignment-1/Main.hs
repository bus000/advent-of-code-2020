...
handleInput :: C.CircularList Int -> IO ()
handleInput
    = mapM_ putStrLn
    . map (concatMap show)
    . take 1
...
step :: C.CircularList Int -> C.CircularList Int
step state = state'''
  where
    (state', next4) = C.removeRightN state 4
    next3 = tail next4
    current = head next4
    state'' = C.insertRight current state'
    insertHere = head . filter (`notElem` next3)
        $ [current-1, current-2..1] ++ [9, 8..1]
    insertPosition = C.seekRight state'' (== insertHere)
    inserted = C.insertLeftN insertPosition (reverse next3)
    state''' = C.moveRight $ C.seekRight inserted (== current)

parseInput :: T.Text -> Either P.ParseError (C.CircularList Int)
parseInput = P.parse (parseCirc <* P.eof) ""
-- parseInput _ = Right $ C.fromList [3, 8, 9, 1, 2, 5, 4, 6, 7]

parseCirc :: P.Parsec T.Text () (C.CircularList Int)
parseCirc = C.fromList . map fromD <$> P.many P.digit <* P.newline
  where
    fromD '0' = 0
    fromD '1' = 1
    fromD '2' = 2
