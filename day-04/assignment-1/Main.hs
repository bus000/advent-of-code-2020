...
newPassport parts = do
    birthYear <- lookup "byr" parts
    issueYear <- lookup "iyr" parts
    expirationYear <- lookup "eyr" parts
    height <- lookup "hgt" parts
    hairColor <- lookup "hcl" parts
    eyeColor <- lookup "ecl" parts
    passportId <- lookup "pid" parts
