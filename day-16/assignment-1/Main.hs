...
handleInput (Input rules _ nearby) = print $ errorRate nearby rules
...
checkRule x (TicketRule intervals) = any (inInterval x) intervals

checkRules :: Int -> [TicketRule] -> Bool
checkRules x rules = any (checkRule x) rules

errorRate :: [Ticket] -> [TicketRule] -> Int
errorRate tickets rules = sum $ map (ticketError rules) tickets

ticketError :: [TicketRule] -> Ticket -> Int
ticketError rules (Ticket values) = sum . filter p $ values
  where
    p x = not $ checkRules x rules
...
parseRule = TicketRule
    <$> (P.manyTill (P.oneOf identifierChars) (P.string ": ") *> parseIntervals)
  where
    identifierChars = ' ':['a'..'z']
...
parseTicket = Ticket <$> P.int `P.sepBy` P.char ','
